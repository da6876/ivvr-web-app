...
class MjrCatController extends Controller
{

    public function index()
    {
        return view('itemsetup.MjrCat.index');
    }
    ...
    public function destroy($id){
        try {
            $permission = MjrCat::findOrFail($id);
            $permission->update([
                'status' => "D",
                'update_by' => auth()->user()->id,
            ]);
    ...
    {
        try {
            $singleDataShow = MjrCat::findOrFail($id);
    ...
    {
        try {
            $singleDataShow = MjrCat::where('status','=','N')->get();
            return $singleDataShow;
    ...
    {
        $query = DB::table('st_mjr_cat as p')
            ->select('p.id','p.mjr_code', 'p.mjr_desc', 'p.status', 'p.create_date')
            ->where('p.status', '!=', 'D');
